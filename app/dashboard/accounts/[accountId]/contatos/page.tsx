...
export default async function ContatosPage({
  ...
  const clickedContactIds = new Set((clicks ?? []).map((c) => c.contact_id));
          ...
          <tbody>
            {contacts.map((c: any) => (
              <tr key={c.id} className="border-t border-[var(--border)]">
                ...
                <td className="px-4 py-3">
                  {clickedContactIds.has(c.id) ? (
                    <span className="pill pill-signal">
                      <span className="pill-dot" /> clicou
                    </span>
                  ) : (
                    <span className="pill pill-neutral">não clicou</span>
                  )}
                </td>
              </tr>
            ))}
