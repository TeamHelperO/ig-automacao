...
import { checkInsightsQuota, logInsightsUsage } from "@/lib/plan-features";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const feature = await checkInsightsQuota(accountId);
  if (!feature.ok) {
    return NextResponse.json({ error: feature.reason }, { status: 403 });
  }
  await logInsightsUsage(accountId);
